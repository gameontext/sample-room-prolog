...
:- module(room_command, [processContent/3,processHelloEvent/2]).
...
processContent("/swagger",JsonDict,Output) :-
    ...
    Content_Pairs = [
	Star-String,
	UserId-"You swagger nonchanlantly about."
    ],
...
processContent("/inventory",JsonDict,Output) :-
    ...
    json_write_dict(Output,_{
    			       type:"event",
    			       content:_{'*':"You try and look at your inventory and realise that state does not yet exist, whether inside or outside of this reality. You scream out to the universe in perplexity, and get told by a still quiet voice to be patient, its being looked at soon."},
    			       bookmark:"not_implemented"
    			   }).
